    ...
    public class XmlTypesBuilder : XmlPatternBuilder {
        ...
        public override void WriteToXml(XmlWriter writer) {
            var xmlType = new XmlType {
                Name = this.Parser.Class.Name,
                Id = this.Parser.Fields.Find(entry => entry.Name == "protocolId").Value,
                Heritage = this.Parser.Class.Heritage,
            };

            if (this.Parser.Class.Name.Contains("CommonIn")) { }

            var xmlFields = new List<XmlField>();

            MethodInfo deserializeAsMethod = this.Parser.Methods.Find(entry => entry.Name.Contains("deserializeAs"));
            string type = null;
            int limit = 0;

            for (int i = 0; i < deserializeAsMethod.Statements.Count; i++) {
                if (deserializeAsMethod.Statements[i] is AssignationStatement && ((AssignationStatement) deserializeAsMethod.Statements[i]).Value.Contains("Read")) {
                    ...
                    if (match.Success) {
                        IEnumerable<string> limitLinq = from entry in this.Parser.Constructors[0].Statements
                                                        where
                                                            entry is AssignationStatement && ((AssignationStatement) entry).Name == match.Groups[1].Value
                                                        let entryMatch =
                    ...
                    FieldInfo field = this.Parser.Fields.Find(entry => entry.Name == name);
                ...
                if (deserializeAsMethod.Statements[i] is InvokeExpression && ((InvokeExpression) deserializeAsMethod.Statements[i]).Name == "deserialize") {
                    var statement = ((InvokeExpression) deserializeAsMethod.Statements[i]);
                    FieldInfo field = this.Parser.Fields.Find(entry => entry.Name == statement.Target);
                    ...
                    if (field != null && xmlFields.Count(entry => entry.Name == field.Name) <= 0) {
                    ...
                    else if (i > 0 && deserializeAsMethod.Statements[i - 1] is AssignationStatement) {
                        ...
                        if (match.Success) {
                            ...
                            if (arrayMatch.Success) {
                                IEnumerable<string> limitLinq = from entry in this.Parser.Constructors[0].Statements
                                                                where
                                                                    entry is AssignationStatement && ((AssignationStatement) entry).Name == arrayMatch.Groups[1].Value
                                                                let entryMatch =
                                                                    Regex.Match(((AssignationStatement) entry).Value,
                                                                                @"new List<[\d\w\._]+>\(([\d]+)\)")
                                                                where entryMatch.Success
                        ...
                        field = this.Parser.Fields.Find(entry => entry.Name == name);
                ...
                if (deserializeAsMethod.Statements[i] is AssignationStatement && ((AssignationStatement) deserializeAsMethod.Statements[i]).Value.Contains("getFlag")) {
                    var statement = ((AssignationStatement) deserializeAsMethod.Statements[i]);
                    FieldInfo field = this.Parser.Fields.Find(entry => entry.Name == statement.Name);
                ...
                if (deserializeAsMethod.Statements[i] is AssignationStatement && ((AssignationStatement) deserializeAsMethod.Statements[i]).Value.Contains("getInstance")) {
                    var statement = ((AssignationStatement) deserializeAsMethod.Statements[i]);
                    FieldInfo field = this.Parser.Fields.Find(entry => entry.Name == statement.Name);
                ...
                if (deserializeAsMethod.Statements[i] is InvokeExpression && ((InvokeExpression) deserializeAsMethod.Statements[i]).Name == "Add" && type != null) {
                    var statement = ((InvokeExpression) deserializeAsMethod.Statements[i]);
