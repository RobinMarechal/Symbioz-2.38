...
namespace Symbioz.ProtocolBuilder.Parsing.Elements {
    public class InvokeExpression : IStatement {
        ...
        public string ReturnVariableTypeAssignation { get; set; }

        public string ReturnVariableAssignationTarget { get; set; }

        public string ReturnVariableAssignation { get; set; }
        ...
        public static InvokeExpression Parse(string str) {
            ...
            if (match.Success) {
                ...
                if (result.ReturnVariableAssignationTarget != null && result.ReturnVariableAssignationTarget.EndsWith("."))
                    result.ReturnVariableAssignationTarget =
                        result.ReturnVariableAssignationTarget.Remove(result.ReturnVariableAssignationTarget.Length - 1, 1); // remove dot

                if (match.Groups["assignation"].Value != "")
                    result.ReturnVariableAssignation =
                        match.Groups["assignation"].Value.Trim().Replace("=", "").Replace(":", "").Replace("*", "").Trim();
