        ...
        public static ControlStatement Parse(string str) {
            ...
            if (match.Success) {
                ...
                if (match.Groups["condition"].Value != "") {
                    ...
                    if (match.Groups["condition"].Value.StartsWith("(") && match.Groups["condition"].Value.EndsWith(")"))
                        result.Condition = match.Groups["condition"].Value.Remove(match.Groups["condition"].Value.Length - 1, 1).Remove(0, 1).Trim();
                    else
