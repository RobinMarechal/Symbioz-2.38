...
namespace Symbioz.Core.DesignPattern.StartupEngine {
    ...
    public class StartupManager : Singleton<StartupManager> {
        ...
        public void Initialize(Assembly startupAssembly) {
            this.logger.Color1("-- Initialisation --");
            ...
            foreach (var pass in Enum.GetValues(typeof(StartupInvokePriority))) {
                foreach (var item in startupAssembly.GetTypes()) {
                    ...
                    var attributes = methods.ConvertAll(x => new KeyValuePair<StartupInvoke, MethodInfo>(x.GetCustomAttribute(typeof(StartupInvoke), false) as StartupInvoke, x))
                                            .FindAll(x => x.Key.Type == (StartupInvokePriority) pass);

                    foreach (var data in attributes) {
                        if (!data.Key.Hided) {
                            this.logger.White("(" + pass + ") Loading " + data.Key.Name + " ...");
                        }
            ...
            watch.Stop();
            this.logger.Color1("-- Initialisation Complete (" + watch.Elapsed.Minutes + "min" + watch.Elapsed.Seconds + "s) --");
        }
    }
}