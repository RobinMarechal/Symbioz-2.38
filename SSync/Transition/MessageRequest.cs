...
namespace SSync.Transition {
    public class MessageRequest<T> : IMessageRequest where T : TransitionMessage {
        ...
        void TimeoutTimer_Elapsed(object sender, ElapsedEventArgs e) {
            this.logger.Alert("Transition Message " + typeof(T).Name + " timeout..");
            this.TimeoutTimer.Stop();

            if (this.ErrorCallback != null) this.ErrorCallback();
        }

        public RequestCallbackDelegate<T> Callback { get; set; }
        public RequestCallbackErrorDelegate ErrorCallback { get; set; }
