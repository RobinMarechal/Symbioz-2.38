...
namespace SSync.Sockets {
    public class BufferManager //: IDisposable
    {
        ...
        public int AvailableSegmentsCount {
            get { return this._availableSegments.Count; } //do we really care about volatility here?
        }
        ...
        public int TotalBufferCount {
            get { return this._buffers.Count; } //do we really care about volatility here?
        }
        ...
        public int TotalSegmentCount {
            get { return this._totalSegmentCount; } //do we really care about volatility here?
        }
        ...
        public int TotalAllocatedMemory {
            get { return this._buffers.Count * (this._segmentCount * this._segmentSize); } // do we really care about volatility here?
        }
        ...
        public BufferSegment CheckOut() {
            ...
            if (!this._availableSegments.TryDequeue(out segment)) {
                lock (this._buffers) {
                    while (!this._availableSegments.TryDequeue(out segment)) {
                        this.CreateBuffer();
                    }
                }
            }
        ...
        private void CreateBuffer() {
            // create a new buffer 
            var newBuf = new ArrayBuffer(this, this._segmentCount * this._segmentSize);

            // create segments from the buffer
            for (int i = 0; i < this._segmentCount; i++) {
                this._availableSegments.Enqueue(new BufferSegment(newBuf, i * this._segmentSize, this._segmentSize, _segmentId++));
            }
            ...
            this._buffers.Add(newBuf);

            // update global alloc'd memory
            Interlocked.Add(ref GlobalAllocatedMemory, this._segmentCount * this._segmentSize);
        }
