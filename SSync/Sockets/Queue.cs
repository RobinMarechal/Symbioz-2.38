    ...
    public class LockfreeQueue<T> : IEnumerable<T> {
        ...
        public void Enqueue(T item) {
            ...
            while (!newNodeWasAdded) {
                ...
                if (this._tail == oldTail) {
                    if (oldTailNext == null) {
                        newNodeWasAdded =
                            Interlocked.CompareExchange<SingleLinkNode<T>>(ref this._tail.Next, newNode, null) == null;
                    }
                    else {
                        Interlocked.CompareExchange<SingleLinkNode<T>>(ref this._tail, oldTailNext, oldTail);
                    }
                }
            }

            Interlocked.CompareExchange<SingleLinkNode<T>>(ref this._tail, newNode, oldTail);
        ...
        public bool TryDequeue(out T item) {
            ...
            while (!haveAdvancedHead) {
                oldHead = this._head;
                SingleLinkNode<T> oldTail = this._tail;
                ...
                if (oldHead == this._head) {
                    if (oldHead == oldTail) {
                        ...
                        Interlocked.CompareExchange<SingleLinkNode<T>>(ref this._tail, oldHeadNext, oldTail);
                    }

                    else {
                        item = oldHeadNext.Item;
                        haveAdvancedHead =
                            Interlocked.CompareExchange<SingleLinkNode<T>>(ref this._head, oldHeadNext, oldHead) == oldHead;
                    }
                }
            }
