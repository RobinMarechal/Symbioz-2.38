...
namespace Symbioz.Network {
    public class AuthClient : SSyncClient {
        ...
        public AuthClient(Socket socket)
            : base(socket) {
            this.Send(new ProtocolRequired(AuthConfiguration.Instance.DofusProtocolVersion, AuthConfiguration.Instance.DofusProtocolVersion));
            this.Send(new RawDataMessage(RawDataManager.GetRawData("auth")));
        }
        ...
        public void SendServerList() {
            this.Send(new ServersListMessage(ServerRecord.GetGameServerInformations(this.Account), 0, true));
        }
        ...
        sbyte[] EncryptTicket() {
            ...
            writer.WriteByte((byte) this.Account.Ticket.Length);
            writer.WriteUTFBytes(this.Account.Ticket);

            return Array.ConvertAll<byte, sbyte>(Cryptography.AESEncrypt(writer.Data, this.AesKey), x => (sbyte) x);
        }

        public bool IsVersionUpToDate() {
            var version = AuthConfiguration.Instance.GetVersionExtended();
            var clientVersion = this.IdentificationMessage.version;
