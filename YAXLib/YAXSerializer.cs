...
namespace YAXLib {
    ...
    public class YAXSerializer {
        ...
        public YAXSerializer(Type t, YAXExceptionHandlingPolicies exceptionPolicy, YAXExceptionTypes defaultExType, YAXSerializationOptions option) {
            ...
            this.m_udtWrapper = TypeWrappersPool.Pool.GetTypeWrapper(this.m_type, this);
            if (this.m_udtWrapper.HasNamespace) this.TypeNamespace = this.m_udtWrapper.Namespace;
            this.MaxRecursion = DefaultMaxRecursion;
        }

        internal XNamespace TypeNamespace { get; set; }

        internal bool HasTypeNamespace {
            get { return this.TypeNamespace.IsEmpty(); }
        }
        ...
        internal void SetNamespaceToOverrideEmptyNamespace(XNamespace otherNamespace) {
            ...
            if (otherNamespace.IsEmpty() && !this.HasTypeNamespace) {
        ...
        public XNamespace YaxLibNamespaceUri {
            get { return this.m_yaxLibNamespaceUri; }
            set { this.m_yaxLibNamespaceUri = value; }
        }
        ...
        public string YaxLibNamespacePrefix {
            get { return this.m_yaxLibNamespacePrefix; }

            set { this.m_yaxLibNamespacePrefix = value; }
        }
        ...
        public string DimentionsAttributeName {
            get { return this.m_dimsAttrName; }

            set { this.m_dimsAttrName = value; }
        }
        ...
        public string RealTypeAttributeName {
            get { return this.m_trueTypeAttrName; }

            set { this.m_trueTypeAttrName = value; }
        }
        ...
        public string Serialize(object obj) {
            return this.SerializeXDocument(obj).ToString();
        }
        ...
        public XDocument SerializeToXDocument(object obj) {
            return this.SerializeXDocument(obj);
        }
        ...
        public object Deserialize(string input) {
            try {
                using (TextReader tr = new StringReader(input)) {
                    ...
                    this.FindDocumentDefaultNamespace();
        ...
        private XDocument SerializeXDocument(object obj) {
            ...
            this.m_isSerializing = true;
            if (this.m_serializedStack == null) this.m_serializedStack = new Stack<object>();
            this.m_mainDocument = new XDocument();
            this.m_mainDocument.Add(this.SerializeBase(obj));
        ...
        private XElement SerializeBase(object obj) {
            if (obj == null)
                return new XElement(this.m_udtWrapper.Alias);
            ...
            if (this.m_udtWrapper.IsTreatedAsDictionary) {
                var elemResult = this.MakeDictionaryElement(null,
                                                            this.m_udtWrapper.Alias,
                                                            obj,
                                                            this.m_udtWrapper.DictionaryAttributeInstance,
                                                            this.m_udtWrapper.CollectionAttributeInstance);
                if (this.m_udtWrapper.PreservesWhitespace)
                    XMLUtils.AddPreserveSpaceAttribute(elemResult);
                if (elemResult.Parent == null) this.AddNamespacesToElement(elemResult);

                return elemResult;
            }
            else if (this.m_udtWrapper.IsTreatedAsCollection) {
                var elemResult = this.MakeCollectionElement(null, this.m_udtWrapper.Alias, obj, null, null);
                if (this.m_udtWrapper.PreservesWhitespace)
                    XMLUtils.AddPreserveSpaceAttribute(elemResult);
                if (elemResult.Parent == null) this.AddNamespacesToElement(elemResult);
            ...
            else if (ReflectionUtils.IsBasicType(this.m_udtWrapper.UnderlyingType)) {
                bool dummyAlreadyAdded;
                var elemResult = this.MakeBaseElement(null, this.m_udtWrapper.Alias, obj, out dummyAlreadyAdded);
                if (this.m_udtWrapper.PreservesWhitespace)
                    XMLUtils.AddPreserveSpaceAttribute(elemResult);
                if (elemResult.Parent == null) this.AddNamespacesToElement(elemResult);
            ...
            else if (!this.m_udtWrapper.UnderlyingType.EqualsOrIsNullableOf(obj.GetType())) {
                ...
                // object of its child
                var ser = this.NewInternalSerializer(obj.GetType(), this.TypeNamespace, null);
                ...
                this.FinalizeNewSerializer(ser, importNamespaces: true, popFromSerializationStack: false);
                elem.Name = this.m_udtWrapper.Alias;

                elem.AddAttributeNamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, obj.GetType().FullName, this.m_documentDefaultNamespace);
                this.RegisterYaxLibNamespace();
                this.AddNamespacesToElement(elem);

                return elem;
            }
            else {
                // SerializeBase will add the object to the stack
                var elem = this.SerializeBase(obj, this.m_udtWrapper.Alias);
                if (!this.m_type.IsValueType) this.m_serializedStack.Pop();
                Debug.Assert(this.m_serializedStack.Count == 0, "Serialization stack is not empty at the end of serialization");
        ...
        private void PushObjectToSerializationStack(object obj) {
            if (!obj.GetType().IsValueType) this.m_serializedStack.Push(obj);
        }

        private void FindDocumentDefaultNamespace() {
            if (this.m_udtWrapper.HasNamespace && String.IsNullOrEmpty(this.m_udtWrapper.NamespacePrefix))
                // it has a default namespace defined (one without a prefix)
                this.m_documentDefaultNamespace = this.m_udtWrapper.Namespace; // set the default namespace
        }
        ...
        private void SetBaseElement(XElement baseElement) {
            this.m_baseElement = baseElement;
        }
        ...
        private XElement SerializeBase(object obj, XName className) {
            this.m_isSerializing = true; // this is set once again here since internal serializers may not call public Serialize methods

            if (this.m_baseElement == null) {
                this.m_baseElement = this.CreateElementWithNamespace(this.m_udtWrapper, className);
            }
            else {
                var baseElem = new XElement(className, null);
                this.m_baseElement.Add(baseElem);
                this.m_baseElement = baseElem;
            }

            if (this.MaxRecursion == 1) {
                this.PushObjectToSerializationStack(obj);

                return this.m_baseElement;
            }

            if (!this.m_type.IsValueType) {
                var alreadySerializedObject = this.m_serializedStack.FirstOrDefault(x => ReferenceEquals(x, obj));
                if (alreadySerializedObject != null) {
                    if (!this.m_udtWrapper.ThrowUponSerializingCyclingReferences) {
            ...
            if (this.m_udtWrapper.HasComment && this.m_baseElement.Parent == null && this.m_mainDocument != null) {
                foreach (string comment in this.m_udtWrapper.Comment) this.m_mainDocument.Add(new XComment(comment));
            }

            // if the containing element is set to preserve spaces, then emit the 
            // required attribute
            if (this.m_udtWrapper.PreservesWhitespace) {
                XMLUtils.AddPreserveSpaceAttribute(this.m_baseElement);
            }

            // check if the main class/type has defined custom serializers
            if (this.m_udtWrapper.HasCustomSerializer) {
                InvokeCustomSerializerToElement(this.m_udtWrapper.CustomSerializerType, obj, this.m_baseElement);
            }
            else if (KnownTypes.IsKnowType(this.m_type)) {
                KnownTypes.Serialize(obj, this.m_baseElement, this.TypeNamespace);
            }
            else // if it has no custom serializers
            {
                ...
                foreach (var member in this.GetFieldsToBeSerialized()) {
                    if (member.HasNamespace) {
                        this.RegisterNamespace(member.Namespace, member.NamespacePrefix);
                    }
                    ...
                    if (elementValue == null && this.m_udtWrapper.IsNotAllowdNullObjectSerialization) {
                        continue;
                    }
                    ...
                    bool isCollectionSerially = member.CollectionAttributeInstance != null && member.CollectionAttributeInstance.SerializationType == YAXCollectionSerializationTypes.Serially;
                    ...
                    if (member.IsSerializedAsAttribute && (areOfSameType || hasCustomSerializer || isCollectionSerially || isKnownType)) {
                        if (!XMLUtils.AttributeExists(this.m_baseElement, serializationLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace))) {
                            XAttribute attrToCreate = XMLUtils.CreateAttribute(this.m_baseElement,
                                                                               serializationLocation,
                                                                               member.Alias.OverrideNsIfEmpty(this.TypeNamespace),
                                                                               (hasCustomSerializer || isCollectionSerially || isKnownType) ? String.Empty : elementValue,
                                                                               this.m_documentDefaultNamespace);

                            this.RegisterNamespace(member.Alias.OverrideNsIfEmpty(this.TypeNamespace).Namespace, null);
                            ...
                            if (member.HasCustomSerializer) {
                                ...
                                var added = this.MakeCollectionElement(tempLoc, "name", elementValue, member.CollectionAttributeInstance, member.Format);
                        ...
                        XElement parElem = XMLUtils.FindLocation(this.m_baseElement, serializationLocation);
                        if (parElem == null) // if the parent element does not exist
                        {
                            // see if the location can be created
                            if (!XMLUtils.CanCreateLocation(this.m_baseElement, serializationLocation))
                                throw new YAXBadLocationException(serializationLocation);
                            // try to create the location
                            parElem = XMLUtils.CreateLocation(this.m_baseElement, serializationLocation);
                    ...
                    else // if the data is going to be serialized as an element
                    {
                        ...
                        if (hasCustomSerializer) {
                            var elemToFill = new XElement(member.Alias.OverrideNsIfEmpty(this.TypeNamespace));
                            ...
                            var elemToFill = new XElement(member.Alias.OverrideNsIfEmpty(this.TypeNamespace));
                            parElem.Add(elemToFill);
                            KnownTypes.Serialize(elementValue, elemToFill, member.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThen(XNamespace.None));
                            if (member.PreservesWhitespace)
                                XMLUtils.AddPreserveSpaceAttribute(elemToFill);
                        }
                        else {
                            ...
                            bool moveDescOnly;
                            bool alreadyAdded;
                            XElement elemToAdd = this.MakeElement(parElem, member, elementValue, out moveDescOnly, out alreadyAdded);
                            if (!areOfSameType) {
                                ...
                                if (realTypeDefinition != null) {
                                    ...
                                    elemToAdd.AddAttributeNamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, realType.FullName, this.m_documentDefaultNamespace);
                                    this.RegisterYaxLibNamespace();
                                }
                            }
                            ...
                            if (moveDescOnly) // if only the descendants of the resulting element are going to be added ...
                            {
                                ...
                                XElement existingElem = parElem.Element(member.Alias.OverrideNsIfEmpty(this.TypeNamespace));
                ...
                if (this.m_baseElement.Parent != null && XMLUtils.IsElementCompletelyEmpty(this.m_baseElement) && isAnythingFoundToSerialize) {
                    this.m_baseElement.Remove();
                }
            } // end of else if it has no custom serializers

            if (this.m_baseElement.Parent == null) {
                this.AddNamespacesToElement(this.m_baseElement);
            }
        ...
        private XElement CreateElementWithNamespace(UdtWrapper wrapper, XName className) {
            ...
            if (elemName.Namespace == wrapper.Namespace)
                this.RegisterNamespace(elemName.Namespace, wrapper.NamespacePrefix);
            else
                this.RegisterNamespace(elemName.Namespace, null);

            return new XElement(elemName, null);
        }

        private void RegisterYaxLibNamespace() {
            this.RegisterNamespace(this.m_yaxLibNamespaceUri, this.m_yaxLibNamespacePrefix);
        }
        ...
        private void RegisterNamespace(XNamespace ns, string prefix) {
            ...
            if (this.m_namespaceToPrefix.ContainsKey(ns)) {
                string existingPrefix = this.m_namespaceToPrefix[ns];
                // override the prefix only if already existing namespace has no prefix assigned
                if (String.IsNullOrEmpty(existingPrefix)) this.m_namespaceToPrefix[ns] = prefix;
            }
            else {
                this.m_namespaceToPrefix.Add(ns, prefix);
            }
        }
        ...
        private void AddNamespacesToElement(XElement rootNode) {
            ...
            foreach (var ns in this.m_namespaceToPrefix.Keys) {
                string prefix = this.m_namespaceToPrefix[ns];
                ...
                else // if it has a prefix assigned
                {
                    ...
                    if (rootNode.GetNamespaceOfPrefix(prefix) == null) {
                        rootNode.AddAttributeNamespaceSafe(XNamespace.Xmlns + prefix, ns, this.m_documentDefaultNamespace);
                    }
                    else // if this prefix is already added
                    {
                        ...
                        if (existing != ns)
                            throw new InvalidOperationException(String.Format("You cannot have two different namespaces with the same prefix." + Environment.NewLine + "Prefix: {0}, Namespaces: \"{1}\", and \"{2}\"",
                                                                              prefix,
                                                                              ns,
                                                                              existing));
                    }
            ...
            if (this.m_udtWrapper.Namespace.IsEmpty() && String.IsNullOrEmpty(this.m_udtWrapper.NamespacePrefix)) {
                // it will be added automatically
                nsNoPrefix.Remove(this.m_udtWrapper.Namespace);
            }

            // now generate namespaces for those without prefix
            foreach (var ns in nsNoPrefix) {
                rootNode.AddAttributeNamespaceSafe(XNamespace.Xmlns + rootNode.GetRandomPrefix(), ns, this.m_documentDefaultNamespace);
            }
        }
        ...
        private XElement MakeElement(XElement insertionLocation, MemberWrapper member, object elementValue, out bool moveDescOnly, out bool alreadyAdded) {
            ...
            if (member.IsTreatedAsDictionary) {
                elemToAdd = this.MakeDictionaryElement(insertionLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace), elementValue, member.DictionaryAttributeInstance, member.CollectionAttributeInstance);
                if (member.CollectionAttributeInstance != null && member.CollectionAttributeInstance.SerializationType == YAXCollectionSerializationTypes.RecursiveWithNoContainingElement && !elemToAdd.HasAttributes)
                    moveDescOnly = true;
                alreadyAdded = elemToAdd.Parent == insertionLocation;
            }
            else if (member.IsTreatedAsCollection) {
                elemToAdd = this.MakeCollectionElement(insertionLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace), elementValue, member.CollectionAttributeInstance, member.Format);
                ...
                elemToAdd = this.MakeBaseElement(insertionLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace), elementValue, out alreadyAdded);
            }
        ...
        private XElement MakeDictionaryElement(XElement insertionLocation,
            ...
            var ser = this.NewInternalSerializer(elementValue.GetType(), elementName.Namespace, insertionLocation);
            XElement elem = ser.SerializeBase(elementValue, elementName);
            this.FinalizeNewSerializer(ser, true);
            ...
            XName keyAlias = elementName.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone() + "Key";
            XName valueAlias = elementName.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone() + "Value";
            ...
            if (collectionAttrInst != null && !String.IsNullOrEmpty(collectionAttrInst.EachElementName)) {
                ...
                if (eachElementName.Namespace.IsEmpty()) this.RegisterNamespace(eachElementName.Namespace, null);
                eachElementName = eachElementName.OverrideNsIfEmpty(elementName.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone());
            }
            ...
            if (dicAttrInst != null) {
                ...
                if (keyAlias.Namespace.IsEmpty()) this.RegisterNamespace(keyAlias.Namespace, null);
                keyAlias = keyAlias.OverrideNsIfEmpty(elementName.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone());

                valueAlias = StringUtils.RefineSingleElement(dicAttrInst.ValueName ?? "Value");
                if (valueAlias.Namespace.IsEmpty()) this.RegisterNamespace(valueAlias.Namespace, null);
                valueAlias = valueAlias.OverrideNsIfEmpty(elementName.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone());
            }
            ...
            foreach (object obj in dicInst) {
                ...
                if (isKeyAttrib && areKeyOfSameType) {
                    elemChild.AddAttributeNamespaceSafe(keyAlias, keyObj, this.m_documentDefaultNamespace);
                }
                else if (isKeyContent && areKeyOfSameType) {
                    elemChild.AddXmlContent(keyObj);
                }
                else {
                    XElement addedElem = this.AddObjectToElement(elemChild, keyAlias, keyObj);
                    if (!areKeyOfSameType) {
                        ...
                        addedElem.AddAttributeNamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, keyObj.GetType().FullName, this.m_documentDefaultNamespace);
                        this.RegisterYaxLibNamespace();
                    }
                }

                if (isValueAttrib && areValueOfSameType) {
                    elemChild.AddAttributeNamespaceSafe(valueAlias, valueObj, this.m_documentDefaultNamespace);
                }
                else if (isValueContent && areValueOfSameType) {
                    elemChild.AddXmlContent(valueObj);
                }
                else {
                    XElement addedElem = this.AddObjectToElement(elemChild, valueAlias, valueObj);
                    if (!areValueOfSameType) {
                        ...
                        addedElem.AddAttributeNamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, valueObj.GetType().FullName, this.m_documentDefaultNamespace);
                        this.RegisterYaxLibNamespace();
                    }
                }
        ...
        private XElement AddObjectToElement(XElement elem, XName alias, object obj) {
            ...
            if (alias == null && udt != null)
                alias = udt.Alias.OverrideNsIfEmpty(this.TypeNamespace);
        ...
        private XElement MakeCollectionElement(XElement insertionLocation,
            ...
            if (serType == YAXCollectionSerializationTypes.Serially && elemToAdd.IsEmpty) {
                ...
                elemToAdd = this.MakeBaseElement(insertionLocation, elementName, sb.ToString(), out alreadyAdded);
                if (alreadyAdded)
                    elemToAdd = null;
            }
            else {
                ...
                foreach (object obj in collectionInst) {
                    ...
                    if (obj != null && !obj.GetType().EqualsOrIsNullableOf(colItemType)) {
                        itemElem.AddAttributeNamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, obj.GetType().FullName, this.m_documentDefaultNamespace);
            ...
            if (arrayDims != null && arrayDims.Length > 1) {
                elemToAdd.AddAttributeNamespaceSafe(this.m_yaxLibNamespaceUri + this.m_dimsAttrName, StringUtils.GetArrayDimsString(arrayDims), this.m_documentDefaultNamespace);
                this.RegisterYaxLibNamespace();
            }
        ...
        private XElement MakeBaseElement(XElement insertionLocation, XName name, object value, out bool alreadyAdded) {
            ...
            if (value == null || ReflectionUtils.IsBasicType(value.GetType())) {
                ...
                var ser = this.NewInternalSerializer(value.GetType(), name.Namespace, insertionLocation);
        ...
        private object DeserializeBase(XElement baseElement) {
            this.doc = baseElement;

            this.m_isSerializing = false;

            if (baseElement == null) {
                return this.m_desObject;
            }

            if (this.m_udtWrapper.HasCustomSerializer) {
                return InvokeCustomDeserializerFromElement(this.m_udtWrapper.CustomSerializerType, baseElement);
            }

            var realTypeAttr = baseElement.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, this.m_documentDefaultNamespace);
            if (realTypeAttr != null) {
                Type theRealType = ReflectionUtils.GetTypeByName(realTypeAttr.Value);
                if (theRealType != null) {
                    this.m_type = theRealType;
                    this.m_udtWrapper = TypeWrappersPool.Pool.GetTypeWrapper(this.m_type, this);
                }
            }
            ...
            if (KnownTypes.IsKnowType(this.m_type)) {
                return KnownTypes.Deserialize(baseElement, this.m_type, this.TypeNamespace);
            }

            if ((this.m_udtWrapper.IsTreatedAsCollection || this.m_udtWrapper.IsTreatedAsDictionary) && !this.IsCraetedToDeserializeANonCollectionMember) {
                if (this.m_udtWrapper.DictionaryAttributeInstance != null) {
                    return this.DeserializeTaggedDictionaryValue(baseElement, this.m_udtWrapper.Alias, this.m_type, this.m_udtWrapper.CollectionAttributeInstance, this.m_udtWrapper.DictionaryAttributeInstance);
                }
                else {
                    return this.DeserializeCollectionValue(this.m_type, baseElement, this.m_udtWrapper.Alias, this.m_udtWrapper.CollectionAttributeInstance);
                }
            }

            if (ReflectionUtils.IsBasicType(this.m_type)) {
                return ReflectionUtils.ConvertBasicType(baseElement.Value, this.m_type);
            }

            object o;
            if (this.m_desObject != null)
                o = this.m_desObject;
            else
                o = this.m_type.InvokeMember(string.Empty, BindingFlags.CreateInstance, null, null, new object[0]);
            ...
            foreach (var member in this.GetFieldsToBeSerialized()) {
                ...
                this.m_exceptionOccurredDuringMemberDeserialization = false;
                ...
                if (member.IsSerializedAsAttribute) {
                    // find the parent element from its location
                    XAttribute attr = XMLUtils.FindAttribute(baseElement, serializationLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace));
                    if (attr == null) // if the parent element does not exist
                    {
                        // loook for an element with the same name AND a yaxlib:realtype attribute
                        XElement elem = XMLUtils.FindElement(baseElement, serializationLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace));
                        if (elem != null && elem.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, this.m_documentDefaultNamespace) != null) {
                            elemValue = elem.Value;
                            xelemValue = elem;
                        }
                        else {
                            this.OnExceptionOccurred(new YAXAttributeMissingException(StringUtils.CombineLocationAndElementName(serializationLocation, member.Alias)),
                                                     (!member.MemberType.IsValueType && this.m_udtWrapper.IsNotAllowdNullObjectSerialization) ? YAXExceptionTypes.Ignore : member.TreatErrorsAs);
                        }
                    }
                    else {
                    ...
                    {
                        this.OnExceptionOccurred(new YAXElementMissingException(serializationLocation),
                                                 (!member.MemberType.IsValueType && this.m_udtWrapper.IsNotAllowdNullObjectSerialization) ? YAXExceptionTypes.Ignore : member.TreatErrorsAs);
                    }
                    else {
                        ...
                        if (values.Length <= 0) {
                            // loook for an element with the same name AND a yaxlib:realtype attribute
                            XElement innerelem = XMLUtils.FindElement(baseElement, serializationLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace));
                            if (innerelem != null && innerelem.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, this.m_documentDefaultNamespace) != null) {
                ...
                else // if member is serialized as an xml element
                {
                    ...
                    if (elem == null) // such element is not found
                    {
                        if ((member.IsTreatedAsCollection || member.IsTreatedAsDictionary)
                            ...
                            if (this.AtLeastOneOfCollectionMembersExists(baseElement, member)) {
                                elem = baseElement;
                                canContinue = true;
                                foundAnyOfMembers = true;
                            }
                            ...
                            XElement fakeElem = XMLUtils.CreateElement(baseElement, serializationLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace));
                            if (fakeElem != null) {
                                createdFakeElement = true;
                                if (this.AtLeastOneOfMembersExists(fakeElem, member.MemberType)) {
                                    canContinue = true;
                                    foundAnyOfMembers = true;
                                    elem = fakeElem;
                                    elemValue = elem.Value;
                        ...
                        //}
                        if (!canContinue) {
                            this.OnExceptionOccurred(new YAXElementMissingException(StringUtils.CombineLocationAndElementName(serializationLocation, member.Alias.OverrideNsIfEmpty(this.TypeNamespace))),
                                                     (!member.MemberType.IsValueType && this.m_udtWrapper.IsNotAllowdNullObjectSerialization) ? YAXExceptionTypes.Ignore : member.TreatErrorsAs);
                        }
                    }
                    else {
                ...
                if (this.m_exceptionOccurredDuringMemberDeserialization) {
                    if (this.m_desObject == null) // i.e. if it was NOT resuming deserialization, set default value, otherwise existing value for the member is kept
                    {
                        if (!member.MemberType.IsValueType && this.m_udtWrapper.IsNotAllowdNullObjectSerialization) {
                            try {
                                member.SetValue(o, null);
                            }
                            catch {
                                this.OnExceptionOccurred(new YAXDefaultValueCannotBeAssigned(member.Alias.LocalName, member.DefaultValue),
                    ...
                    this.RetreiveElementValue(o, member, elemValue, xelemValue);
                    if (createdFakeElement && !this.m_exceptionOccurredDuringMemberDeserialization)
                        foundAnyOfMembers = true;
                }
                ...
                if (this.RemoveDeserializedXmlNodes) {
        ...
        private bool AtLeastOneOfCollectionMembersExists(XElement elem, MemberWrapper member) {
            ...
            return (elem.Element(eachElementName.OverrideNsIfEmpty(member.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone())) != null);
        }
        ...
        private bool AtLeastOneOfMembersExists(XElement elem, Type type) {
            ...
            foreach (var member in this.GetFieldsToBeSerialized(typeWrapper)) {
                if (!member.CanWrite)
                    continue;
                ...
                if (member.IsSerializedAsAttribute) {
                    ...
                    if (attr == null) {
                        ...
                        if (theElem != null && theElem.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, this.m_documentDefaultNamespace) != null)
                            return true;
                    }
                    else {
                        return true;
                    ...
                    if (xelem == null) {
                        if (!ReflectionUtils.IsBasicType(member.MemberType)
                            ...
                            && member.MemberType != this.m_type) // searching for same type objects will lead to infinite loops
                        {
                            // try to create a fake element 
                            XElement fakeElem = XMLUtils.CreateElement(elem, member.SerializationLocation, member.Alias);
                            if (fakeElem != null) {
                                bool memberExists = this.AtLeastOneOfMembersExists(fakeElem, member.MemberType);
                                fakeElem.Remove();
        ...
        private void RetreiveElementValue(object o, MemberWrapper member, string elemValue, XElement xelemValue) {
            ...
            if (xelemValue != null && !isRealTypeAttributeNotRelevant) {
                XAttribute realTypeAttribute = xelemValue.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, this.m_documentDefaultNamespace);
            ...
            if (xelemValue != null
                && XMLUtils.IsElementCompletelyEmpty(xelemValue)
                && !ReflectionUtils.IsBasicType(memberType)
                && !member.IsTreatedAsCollection
                && !member.IsTreatedAsDictionary
                && !this.AtLeastOneOfMembersExists(xelemValue, memberType)) {
                try {
                    member.SetValue(o, member.DefaultValue);
                }
                catch {
                    this.OnExceptionOccurred(new YAXDefaultValueCannotBeAssigned(member.Alias.LocalName, member.DefaultValue), member.TreatErrorsAs);
                }
            }
                ...
                try {
                    ...
                    try {
                        ...
                        this.OnExceptionOccurred(new YAXDefaultValueCannotBeAssigned(member.Alias.LocalName, member.DefaultValue), this.m_defaultExceptionType);
                    }
                }
            }
            else if (member.IsTreatedAsDictionary && member.DictionaryAttributeInstance != null) {
                this.DeserializeTaggedDictionaryMember(o, member, xelemValue);
            }
            else if (member.IsTreatedAsCollection) {
                this.DeserializeCollectionMember(o, member, memberType, elemValue, xelemValue);
            }
            else {
                var namespaceToOverride = member.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone();
                var ser = this.NewInternalSerializer(memberType, namespaceToOverride, null);

                ser.IsCraetedToDeserializeANonCollectionMember = !(member.IsTreatedAsDictionary || member.IsTreatedAsCollection);

                if (this.m_desObject != null) // i.e. it is in resuming mode
                    ser.SetDeserializationBaseObject(member.GetValue(o));

                object convertedObj = ser.DeserializeBase(xelemValue);
                this.FinalizeNewSerializer(ser, false);
        ...
        private object DeserializeCollectionValue(Type colType, XElement xelemValue, XName memberAlias, YAXCollectionAttribute colAttrInstance) {
            ...
            if (ReflectionUtils.IsInstantiableCollection(colType)) {
                var namespaceToOverride = memberAlias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone();

                var containerSer = this.NewInternalSerializer(colType, namespaceToOverride, null);
                ...
                this.FinalizeNewSerializer(containerSer, false);
            }
            ...
            if (ReflectionUtils.IsBasicType(itemType) && colAttrInstance != null && colAttrInstance.SerializationType == YAXCollectionSerializationTypes.Serially) {
                ...
                foreach (string wordItem in items) {
                    try {
                        ...
                        this.OnExceptionOccurred(new YAXBadlyFormedInput(memberAlias.ToString(), elemValue), this.m_defaultExceptionType);
                    }
                }
            }
            else //if the collection was serialized recursively
            {
                ...
                if (colAttrInstance != null && colAttrInstance.EachElementName != null) {
                    ...
                    eachElemName = eachElemName.OverrideNsIfEmpty(memberAlias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone());
                }
                ...
                foreach (XElement childElem in elemsToSearch) {
                    ...
                    XAttribute realTypeAttribute = childElem.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, this.m_documentDefaultNamespace);
                    if (realTypeAttribute != null) {
                    ...
                    if (curElementIsPrimitive) {
                        ...
                        var namespaceToOverride = memberAlias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone();
                        var ser = this.NewInternalSerializer(curElementType, namespaceToOverride, null);
            ...
            if (ReflectionUtils.IsArray(colType)) {
                XAttribute dimsAttr = xelemValue.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_dimsAttrName, this.m_documentDefaultNamespace);
                int[] dims = new int[0];
                if (dimsAttr != null) {
                ...
                if (dims.Length > 0) {
                    ...
                    for (int i = 0; i < count; i++) {
                        ...
                        try {
                            ...
                            this.OnExceptionOccurred(new YAXCannotAddObjectToCollection(memberAlias.ToString(), lst[i]),
                                                     this.m_defaultExceptionType);
                        }
                    }
                }
            ...
            else if (ReflectionUtils.IsTypeEqualOrInheritedFromType(colType, typeof(Stack)) || ReflectionUtils.IsTypeEqualOrInheritedFromType(colType, typeof(Stack<>))) {
                ...
                if (ReflectionUtils.IsTypeEqualOrInheritedFromType(colType, typeof(Queue)) || ReflectionUtils.IsTypeEqualOrInheritedFromType(colType, typeof(Queue<>))) {
        ...
        private void DeserializeCollectionMember(object o, MemberWrapper member, Type colType, string elemValue, XElement xelemValue) {
            ...
            if (member.CollectionAttributeInstance != null
                ...
                colObject = this.DeserializeCollectionValue(colType, new XElement("temp", elemValue), "temp", member.CollectionAttributeInstance);
            }
            else {
                XName memberAlias = member.Alias.OverrideNsIfEmpty(this.TypeNamespace);
                colObject = this.DeserializeCollectionValue(colType, xelemValue, memberAlias, member.CollectionAttributeInstance);
            }
        ...
        private object DeserializeTaggedDictionaryValue(XElement xelemValue,
            ...
            var namespaceToOverride = alias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone();
            var containerSer = this.NewInternalSerializer(type, namespaceToOverride, null);
            containerSer.IsCraetedToDeserializeANonCollectionMember = true;
            containerSer.RemoveDeserializedXmlNodes = true;
            ...
            bool isKeyContent = false;
            bool isValueContent = false;
            XName keyAlias = alias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone() + "Key";
            XName valueAlias = alias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone() + "Value";
            ...
            if (colAttributeInstance != null && colAttributeInstance.EachElementName != null) {
                ...
                eachElementName = eachElementName.OverrideNsIfEmpty(alias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone());
            }
            ...
            if (dicAttrInstance != null) {
                ...
                if (dicAttrInstance.KeyName != null) {
                    ...
                    keyAlias = keyAlias.OverrideNsIfEmpty(alias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone());
                }

                if (dicAttrInstance.ValueName != null) {
                    valueAlias = StringUtils.RefineSingleElement(dicAttrInstance.ValueName);
                    valueAlias = valueAlias.OverrideNsIfEmpty(alias.Namespace.IfEmptyThen(this.TypeNamespace).IfEmptyThenNone());
                }
            }
            ...
            foreach (XElement childElem in xelemValue.Elements(eachElementName)) {
                ...
                bool isKeyFound = this.VerifyDictionaryPairElements(ref keyType, ref isKeyAttrib, ref isKeyContent, keyAlias, childElem);
                bool isValueFound = this.VerifyDictionaryPairElements(ref valueType, ref isValueAttrib, ref isValueContent, valueAlias, childElem);
                ...
                if (isKeyFound) {
                    if (isKeyAttrib) {
                        key = ReflectionUtils.ConvertBasicType(childElem.Attribute_NamespaceSafe(keyAlias, this.m_documentDefaultNamespace).Value, keyType);
                    }
                        ...
                        if (keySer == null)
                            keySer = this.NewInternalSerializer(keyType, keyAlias.Namespace, null);

                        key = keySer.DeserializeBase(childElem.Element(keyAlias));
                        this.FinalizeNewSerializer(keySer, false);
                    }
                }

                if (isValueFound) {
                    if (isValueAttrib) {
                        value = ReflectionUtils.ConvertBasicType(childElem.Attribute_NamespaceSafe(valueAlias, this.m_documentDefaultNamespace).Value, valueType);
                    }
                        ...
                        if (valueSer == null)
                            valueSer = this.NewInternalSerializer(valueType, valueAlias.Namespace, null);

                        value = valueSer.DeserializeBase(childElem.Element(valueAlias));
                        this.FinalizeNewSerializer(valueSer, false);
                    }
                }
                ...
                try {
                    ...
                    this.OnExceptionOccurred(new YAXCannotAddObjectToCollection(alias.LocalName, new KeyValuePair<object, object>(key, value)),
                                             this.m_defaultExceptionType);
                }
            }
        ...
        private YAXSerializer NewInternalSerializer(Type type, XNamespace namespaceToOverride, XElement insertionLocation) {
            var serializer = new YAXSerializer(type, this.m_exceptionPolicy, this.m_defaultExceptionType, this.m_serializationOption);
            serializer.MaxRecursion = this.MaxRecursion == 0 ? 0 : this.MaxRecursion - 1;
            serializer.m_serializedStack = this.m_serializedStack;
            serializer.m_documentDefaultNamespace = this.m_documentDefaultNamespace;
        ...
        private void FinalizeNewSerializer(YAXSerializer serializer, bool importNamespaces, bool popFromSerializationStack = true) {
            ...
            if (popFromSerializationStack && this.m_isSerializing && serializer.m_type != null && !serializer.m_type.IsValueType) this.m_serializedStack.Pop();

            if (importNamespaces) this.ImportNamespaces(serializer);
            this.m_parsingErrors.AddRange(serializer.ParsingErrors);
        }
        ...
        private void DeserializeTaggedDictionaryMember(object o, MemberWrapper member, XElement xelemValue) {
            object dic = this.DeserializeTaggedDictionaryValue(xelemValue,
                                                               member.Alias,
                                                               member.MemberType,
        ...
        private bool VerifyDictionaryPairElements(ref Type keyType, ref bool isKeyAttrib, ref bool isKeyContent, XName keyAlias, XElement childElem) {
            ...
            if (isKeyAttrib && childElem.Attribute_NamespaceSafe(keyAlias, this.m_documentDefaultNamespace) != null) {
                isKeyFound = true;
            }
                ...
                if (elem != null) {
                    XAttribute realTypeAttr = elem.Attribute_NamespaceSafe(this.m_yaxLibNamespaceUri + this.m_trueTypeAttrName, this.m_documentDefaultNamespace);
                    if (realTypeAttr != null) {
                        Type theRealType = ReflectionUtils.GetTypeByName(realTypeAttr.Value);
                        if (theRealType != null) {
                            keyType = theRealType;
        ...
        private object DeserializeKeyValuePair(XElement baseElement) {
            ...
            XName xnameKey = this.TypeNamespace.IfEmptyThenNone() + "Key";
            XName xnameValue = this.TypeNamespace.IfEmptyThenNone() + "Value";
            ...
            if (ReflectionUtils.IsBasicType(keyType)) {
                ...
                keyValue = this.DeserializeCollectionValue(keyType,
                                                           baseElement.Element(xnameKey),
                                                           xnameKey,
                                                           null);
            }
            else {
                var ser = this.NewInternalSerializer(keyType, xnameKey.Namespace.IfEmptyThenNone(), null);
            ...
            if (ReflectionUtils.IsBasicType(valueType)) {
                ...
                valueValue = valueType.InvokeMember(string.Empty,
                                                    BindingFlags.CreateInstance,
                                                    null,
                                                    null,
                                                    new object[] {baseElement.Element(xnameValue).Value});
            }
            else if (ReflectionUtils.IsCollectionType(valueType)) {
                valueValue = this.DeserializeCollectionValue(valueType,
                                                             baseElement.Element(xnameValue),
                                                             xnameValue,
                                                             null);
            }
            else {
                var ser = this.NewInternalSerializer(valueType, xnameValue.Namespace.IfEmptyThenNone(), null);
                valueValue = ser.DeserializeBase(baseElement.Element(xnameValue));
                this.FinalizeNewSerializer(ser, false);
            }

            object pair = this.m_type.InvokeMember(string.Empty,
                                                   BindingFlags.CreateInstance,
                                                   null,
                                                   null,
        ...
        private IEnumerable<MemberWrapper> GetFieldsToBeSerialized(UdtWrapper typeWrapper) {
            foreach (var member in typeWrapper.UnderlyingType.GetMembers(BindingFlags.Instance | BindingFlags.NonPublic | BindingFlags.Public)) {
                ...
                if ((Char.IsLetter(name0) || name0 == '_')
                    ...
                    if (memInfo.IsAllowedToBeSerialized(typeWrapper.FieldsToSerialize, this.m_udtWrapper.DontSerializePropertiesWithNoSetter)) {
        ...
        private IEnumerable<MemberWrapper> GetFieldsToBeSerialized() {
            return this.GetFieldsToBeSerialized(this.m_udtWrapper).OrderBy(t => t.Order);
        }
        ...
        private void OnExceptionOccurred(YAXException ex, YAXExceptionTypes exceptionType) {
            this.m_exceptionOccurredDuringMemberDeserialization = true;
            ...
            if ((this.m_exceptionPolicy == YAXExceptionHandlingPolicies.ThrowWarningsAndErrors) || (this.m_exceptionPolicy == YAXExceptionHandlingPolicies.ThrowErrorsOnly && exceptionType == YAXExceptionTypes.Error)) {
                throw ex;
            }
        }
    }
