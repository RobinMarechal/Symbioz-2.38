...
namespace YAXLib {
    ...
    internal class MemberWrapper {
        ...
        public MemberWrapper(MemberInfo memberInfo, YAXSerializer callerSerializer) {
            ...
            this.m_isProperty = (memberInfo.MemberType == MemberTypes.Property);

            this.Alias = StringUtils.RefineSingleElement(this.m_memberInfo.Name);

            if (this.m_isProperty)
                this.m_propertyInfoInstance = (PropertyInfo) memberInfo;
            else
                this.m_fieldInfoInstance = (FieldInfo) memberInfo;

            this.m_memberType = this.m_isProperty ? this.m_propertyInfoInstance.PropertyType : this.m_fieldInfoInstance.FieldType;

            this.m_memberTypeWrapper = TypeWrappersPool.Pool.GetTypeWrapper(this.MemberType, callerSerializer);
            if (this.m_memberTypeWrapper.HasNamespace) {
                this.Namespace = this.m_memberTypeWrapper.Namespace;
                this.NamespacePrefix = this.m_memberTypeWrapper.NamespacePrefix;
            }

            this.InitInstance();

            this.TreatErrorsAs = callerSerializer != null ? callerSerializer.DefaultExceptionType : YAXExceptionTypes.Error;
            ...
            foreach (var attrType in attrsToProcessEarlier) {
                var customSerAttrs = Attribute.GetCustomAttributes(this.m_memberInfo, attrType, true);
                foreach (var attr in customSerAttrs) {
                    this.ProcessYaxAttribute(attr);
                }
            }

            foreach (var attr in Attribute.GetCustomAttributes(this.m_memberInfo, true)) {
            ...
            if (this.m_collectionAttributeInstance == null && this.m_memberTypeWrapper.CollectionAttributeInstance != null) this.m_collectionAttributeInstance = this.m_memberTypeWrapper.CollectionAttributeInstance;

            if (this.m_dictionaryAttributeInstance == null && this.m_memberTypeWrapper.DictionaryAttributeInstance != null) this.m_dictionaryAttributeInstance = this.m_memberTypeWrapper.DictionaryAttributeInstance;
        }
        ...
        public bool CanRead {
            get {
                if (this.m_isProperty) {
                    return this.m_propertyInfoInstance.CanRead;
                }
                else {
                    return true;
                }
        ...
        public bool CanWrite {
            get {
                if (this.m_isProperty) {
                    return this.m_propertyInfoInstance.CanWrite;
                }
                else {
                    return true;
                }
        ...
        public bool IsSerializedAsAttribute {
            get { return this.m_isSerializedAsAttribute; }

            private set {
                this.m_isSerializedAsAttribute = value;
                if (value) {
                    // a field cannot be both serialized as an attribute and a value
                    this.m_isSerializedAsValue = false;
                }
            }
        }
        ...
        public bool IsSerializedAsValue {
            get { return this.m_isSerializedAsValue; }

            private set {
                this.m_isSerializedAsValue = value;
                if (value) {
                    // a field cannot be both serialized as an attribute and a value
                    this.m_isSerializedAsAttribute = false;
                }
            }
        }
        ...
        public bool IsSerializedAsElement {
            get { return !this.IsSerializedAsAttribute && !this.IsSerializedAsValue; }
        ...
        public UdtWrapper MemberTypeWrapper {
            get { return this.m_memberTypeWrapper; }
        }
        ...
        public bool IsKnownType {
            get { return this.m_memberTypeWrapper.IsKnownType; }
        }
        ...
        public string SerializationLocation {
            get { return this.m_serializationLocation; }

            private set { this.m_serializationLocation = StringUtils.RefineLocationString(value); }
        }
        ...
        public bool IsTreatedAsCollection {
            get { return !this.IsAttributedAsNotCollection && this.m_memberTypeWrapper.IsTreatedAsCollection; }
        }
        ...
        public bool IsTreatedAsDictionary {
            get { return !this.IsAttributedAsNotCollection && this.m_memberTypeWrapper.IsTreatedAsDictionary; }
        }
        ...
        public bool IsRealTypeDefined(Type type) {
            return this.GetRealTypeDefinition(type) != null;
        }


        // TODO: move to public methods section
        public YAXTypeAttribute GetRealTypeDefinition(Type type) {
            return this.m_possibleRealTypes.FirstOrDefault(x => ReferenceEquals(x.Type, type));
        }
        ...
        public object GetOriginalValue(object obj, object[] index) {
            if (this.m_isProperty) {
                return this.m_propertyInfoInstance.GetValue(obj, index);
            }
            else {
                return this.m_fieldInfoInstance.GetValue(obj);
            }
        }
        ...
        public object GetValue(object obj) {
            object elementValue = this.GetOriginalValue(obj, null);

            if (elementValue == null)
                return null;

            if (this.m_memberTypeWrapper.IsEnum) {
                return this.m_memberTypeWrapper.EnumWrapper.GetAlias(elementValue);
            }
        ...
        public void SetValue(object obj, object value) {
            if (this.m_isProperty) {
                this.m_propertyInfoInstance.SetValue(obj, value, null);
            }
            else {
                this.m_fieldInfoInstance.SetValue(obj, value);
            }
        }
        ...
        public bool IsAllowedToBeSerialized(YAXSerializationFields serializationFields, bool dontSerializePropertiesWithNoSetter) {
            if (dontSerializePropertiesWithNoSetter && this.m_isProperty && !this.m_propertyInfoInstance.CanWrite)
                return false;
            ...
            if (serializationFields == YAXSerializationFields.AllFields)
                ...
                return !this.IsAttributedAsDontSerialize && this.m_isProperty && ReflectionUtils.IsPublicProperty(this.m_propertyInfoInstance);
            else
        ...
        public override string ToString() {
            return this.m_memberInfo.ToString();
        }
        ...
        private void InitDefaultValue() {
            if (this.MemberType.IsValueType)
                this.DefaultValue = this.MemberType.InvokeMember(string.Empty, BindingFlags.CreateInstance, null, null, new object[0]);
            else
        ...
        private void ProcessYaxAttribute(object attr) {
            if (attr is YAXCommentAttribute) {
                ...
                if (ReflectionUtils.IsBasicType(this.MemberType)
                    ...
                    || (this.m_collectionAttributeInstance != null && this.m_collectionAttributeInstance.SerializationType == YAXCollectionSerializationTypes.Serially)) {
                ...
                if (ReflectionUtils.IsBasicType(this.MemberType)
                    ...
                    this.SerializationLocation = path;
                    if (!String.IsNullOrEmpty(alias)) this.Alias = StringUtils.RefineSingleElement(alias);
                }
            }
                ...
                if (!ReflectionUtils.IsArray(this.MemberType)) this.IsAttributedAsNotCollection = true;
            }
                ...
                if (this.m_possibleRealTypes.Any(x => x.Type == yaxTypeAttr.Type))
                    throw new YAXPolymorphicException(String.Format("The type \"{0}\" for field/property \"{1}\" has already been defined through another attribute.", yaxTypeAttr.Type.Name, this.m_memberInfo));

                if (alias != null && this.m_possibleRealTypes.Any(x => alias.Equals(x.Alias, StringComparison.Ordinal)))
                    throw new YAXPolymorphicException(String.Format("The alias \"{0}\" given to type \"{1}\" for field/property \"{2}\" has already been given to another type through another attribute.",
                                                                    alias,
                                                                    yaxTypeAttr.Type.Name,
                                                                    this.m_memberInfo));

                this.m_possibleRealTypes.Add(yaxTypeAttr);
            }
                ...
                if (this.m_possibleCollectionItemRealTypes.Any(x => x.Type == yaxColletionItemTypeAttr.Type))
                    throw new YAXPolymorphicException(String.Format("The collection-item type \"{0}\" for collection \"{1}\" has already been defined through another attribute.",
                                                                    yaxColletionItemTypeAttr.Type.Name,
                                                                    this.m_memberInfo));

                if (alias != null && this.m_possibleCollectionItemRealTypes.Any(x => alias.Equals(x.Alias, StringComparison.Ordinal)))
                ...
                this.m_possibleCollectionItemRealTypes.Add(yaxColletionItemTypeAttr);
            }
            else if (attr is YAXDontSerializeIfNullAttribute) {
                this.IsAttributedAsDontSerializeIfNull = true;
            }
            else if (attr is YAXElementOrder) {
                this.Order = (attr as YAXElementOrder).Order;
            }
            else {
