    ...
    internal class EnumWrapper {
            ...
            foreach (var m in t.GetFields()) {
                if (m.FieldType == t) {
                    ...
                    if (alias != originalName) {
                        if (!this.m_enumMembers.ContainsKey(alias))
                            this.m_enumMembers.Add(m.Name, alias);
                        else if (!this.m_enumMembers.ContainsKey(originalName))
                            this.m_enumMembers.Add(m.Name, originalName);
                        else
        ...
        public object ParseAlias(string alias) {
            ...
            if (components.Length > 0) {
                ...
                string realName = this.FindRealNameFromAlias(components[0]);
                sb.Append(realName);

                for (int i = 1; i < components.Length; i++) {
                    sb.Append(", ");
                    realName = this.FindRealNameFromAlias(components[i]);
                    sb.Append(realName);
                }

                return Enum.Parse(this.m_enumType, sb.ToString());
            }
        ...
        public object GetAlias(object enumMember) {
            ...
            if (components.Length == 1) {
                ...
                if (this.m_enumMembers.TryGetValue(originalName, out alias))
                    return alias;
                else
                    return enumMember;
            }
                ...
                for (int i = 0; i < components.Length; i++) {
                    ...
                    string alias;
                    if (this.m_enumMembers.TryGetValue(components[i], out alias))
                        result.Append(alias);
                    else
        ...
        private string FindRealNameFromAlias(string alias) {
            alias = alias.Trim();
            foreach (var pair in this.m_enumMembers) {
                if (pair.Value == alias)
                    return pair.Key;
            }
