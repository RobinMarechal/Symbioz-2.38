...
namespace Symbioz.World.Records.Items {
    ...
    public class ItemRecord : ITable {
        ...
        public bool HasSet {
            get { return ItemSetRecord.GetItemSet(Id) != null; }
        }

        public ItemSetRecord ItemSet {
            get { return ItemSetRecord.GetItemSet(Id); }
        }

        public bool Weapon {
            get { return WeaponRecord.Weapons.Find(x => x.Id == Id) != null; }
        }

        public ItemRecord(ushort id,
                          string name,
                          ushort typeId,
                          ushort apparenceId,
                          ushort level,
        ...
        public CharacterItemRecord GetCharacterItem(long characterId, uint quantity, bool perfect = false) {
            var effects = Effects.ConvertAll(x => x.GenerateEffect(perfect));
            effects.RemoveAll(x => x == null);
            var item = new CharacterItemRecord(characterId,
                                               0,
                                               Id,
                                               (byte) CharacterInventoryPositionEnum.INVENTORY_POSITION_NOT_EQUIPED,
                                               quantity,
                                               effects,
                                               AppearanceId);
            return item;
        ...
        public ObjectItemToSellInNpcShop GetObjectItemToSellInNpcShop(bool levelPrice) {
            return new ObjectItemToSellInNpcShop(Id,
                                                 ...
                                                 (uint) GetPrice(levelPrice),
                                                 string.Empty);
        }

        public ObjectItemNotInContainer GetObjectItemNotInContainer(uint uid, uint quantity) {
            return new ObjectItemNotInContainer(Id,
                                                Effects.ConvertAll<ObjectEffect>(x => x.GetTemplateObjectEffect()).ToArray(),
                                                uid,
                                                quantity);
        }

        public ObjectItemInformationWithQuantity GetObjectItemInformationWithQuantity(uint quantity) {
            return new ObjectItemInformationWithQuantity(Id,
                                                         Effects.ConvertAll<ObjectEffect>(x => x.GetTemplateObjectEffect()).ToArray(),
                                                         quantity);
        }
