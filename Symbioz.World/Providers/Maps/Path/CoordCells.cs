    ...
    public static class CoordCells {
        ...
        private static void SearNeighbors() {
            foreach (var cell in Cells) {
                // lignes 
                var cells = Cells.FindAll(x => (x.X == cell.X - 1 && x.Y == cell.Y) || (x.X == cell.X + 1 && x.Y == cell.Y) || (x.Y == cell.Y - 1 && x.X == cell.X) || (x.Y == cell.Y + 1 && x.X == cell.X));
                cell.Line = cells;

                // diagonales
                cells = Cells.FindAll(x => (x.X == cell.X - 1 && x.Y == cell.Y - 1) || (x.X == cell.X - 1 && x.Y == cell.Y + 1) || (x.X == cell.X + 1 && x.Y == cell.Y - 1) || (x.X == cell.X + 1 && x.Y == cell.Y + 1));
