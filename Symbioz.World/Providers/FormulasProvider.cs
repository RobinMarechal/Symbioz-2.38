...
namespace Symbioz.World.Providers {
    public class FormulasProvider : Singleton<FormulasProvider> {
        ...
        public uint TotalInitiative(Stats stats) {
            double num1 = stats.Total() + (double) stats.Initiative.Total();
        ...
        public ushort GetZaapCost(MapRecord zaapMap, MapRecord currentMap) {
            ...
            return (ushort) Math.Floor(Math.Sqrt((position2.X - position.X) * (position2.X - position.X) + (position2.Y - position.Y) * (position2.Y - position.Y)) * 10.0);
        }
        ...
        public uint AdjustDroppedKamas(IFightResult looter, int teamPP, long baseKamas, int dropBonusPercent) {
            ...
            double num = (looter.Fight.AgeBonus <= 0) ? 1.0 : (1.0 + looter.Fight.AgeBonus / 100.0);
            int kamas = (int) (baseKamas * (prospecting / (double) teamPP) * num * WorldConfiguration.Instance.KamasRate);
        ...
        public int GetCraftXpByJobLevel(ushort resultLevel, ushort crafterLevel, int craftXpRatio) {
            ...
            value = 20.0 * resultLevel / (Math.Pow(crafterLevel - (double) resultLevel, 1.1) / 10.0 + 1.0);

            if (craftXpRatio > -1) {
                result = value * (craftXpRatio / (double) 100);
            }
        ...
        public uint GetCollectedItemQuantity(short jobLevel, SkillRecord skillRecord) {
            return (uint) new AsyncRandom().Next(jobLevel == 200 ? 7 : 1, skillRecord.MinLevel == 200 ? 1 : 7 + ((jobLevel - skillRecord.MinLevel) / 10));
        }
        ...
        private static short GetJetDelta(Fighter fighter, short jet, EffectElementType elementType, bool useWeapon) {
            ...
            switch (elementType) {
                case EffectElementType.Neutral:
                    result = (short) (jet
                                      * (100
                                         + fighter.Stats.Strength.TotalInContext()
                                         + fighter.Stats.DamagesBonusPercent.TotalInContext()
                                         + (useWeapon ? fighter.Stats.WeaponDamagesBonusPercent.TotalInContext() : 0))
                                      / 100.0
                                      + (fighter.Stats.AllDamagesBonus.TotalInContext() + fighter.Stats.NeutralDamageBonus.TotalInContext()));
                ...
                case EffectElementType.Earth:
                    result = (short) (jet
                                      * (100
                                         ...
                                         + (useWeapon ? fighter.Stats.WeaponDamagesBonusPercent.TotalInContext() : 0))
                                      / 100.0
                                      + (fighter.Stats.AllDamagesBonus.TotalInContext() + fighter.Stats.EarthDamageBonus.TotalInContext()));

                    break;
                case EffectElementType.Water:
                    result = (short) (jet
                                      * (100
                                         + fighter.Stats.Chance.TotalInContext()
                                         + fighter.Stats.DamagesBonusPercent.TotalInContext()
                                         + (useWeapon ? fighter.Stats.WeaponDamagesBonusPercent.TotalInContext() : 0))
                                      / 100.0
                                      + (fighter.Stats.AllDamagesBonus.TotalInContext() + fighter.Stats.WaterDamageBonus.TotalInContext()));

                    break;
                case EffectElementType.Air:
                    result = (short) (jet
                                      * (100
                                         + fighter.Stats.Agility.TotalInContext()
                                         + fighter.Stats.DamagesBonusPercent.TotalInContext()
                                         + (useWeapon ? fighter.Stats.WeaponDamagesBonusPercent.TotalInContext() : 0))
                                      / 100.0
                                      + (fighter.Stats.AllDamagesBonus.TotalInContext() + fighter.Stats.AirDamageBonus.TotalInContext()));

                    break;
                case EffectElementType.Fire:
                    result = (short) (jet
                                      * (100
                                         + fighter.Stats.Intelligence.TotalInContext()
                                         + fighter.Stats.DamagesBonusPercent.TotalInContext()
                                         + (useWeapon ? fighter.Stats.WeaponDamagesBonusPercent.TotalInContext() : 0))
                                      / 100.0
                                      + (fighter.Stats.AllDamagesBonus.TotalInContext() + fighter.Stats.FireDamageBonus.TotalInContext()));
        ...
        public short GetPushDamages(Fighter source, Fighter target, short cellsCount, bool headOn) {
            ...
            double num2 = ((source.Level / (double) 2) + (source.Stats.PushDamageBonus.TotalInContext() - target.Stats.PushDamageReduction.TotalInContext()) + 32)
                          * (cellsCount / num1);
        ...
        public short GetHealDelta(Fighter source, short jet) {
            return (short) (jet * (double) ((100 + source.Stats.Intelligence.TotalInContext())) / 100 + source.Stats.HealBonus.TotalInContext());
        }
        ...
        public bool RollMpLose(Fighter from, Fighter to) {
            double currentActionPointsTo = to.Stats.ActionPoints.TotalInContext();

            double maxActionPointsTo = to.Stats.ActionPoints.Total();

            double mpDodgeFrom = (@from.Stats.Wisdom.TotalInContext() / (double) 10) + @from.Stats.DodgePMProbability.TotalInContext();

            double mpDodgeTo = (to.Stats.Wisdom.TotalInContext() / (double) 10) + to.Stats.DodgePMProbability.TotalInContext();

            mpDodgeTo = mpDodgeTo == 0 ? 1 : mpDodgeTo;
            mpDodgeFrom = mpDodgeFrom == 0 ? 1 : mpDodgeFrom;

            double probability = (currentActionPointsTo / maxActionPointsTo) * (mpDodgeFrom / mpDodgeTo) * (1 / (double) 2);
        ...
        public bool RollApLose(Fighter from, Fighter to) {
            ...
            double apDodgeFrom = (@from.Stats.Wisdom.TotalInContext() / (double) 10) + @from.Stats.DodgePAProbability.TotalInContext();

            double apDodgeTo = (to.Stats.Wisdom.TotalInContext() / (double) 10) + to.Stats.DodgePAProbability.TotalInContext();

            apDodgeTo = apDodgeTo == 0 ? 1 : apDodgeTo;
            apDodgeFrom = apDodgeFrom == 0 ? 1 : apDodgeFrom;

            double probability = (currentActionPointsTo / maxActionPointsTo) * (apDodgeFrom / apDodgeTo) * (1 / (double) 2);
        ...
        public int GetSummonedCharacteristicDelta(int baseDelta, ushort ownerLevel) {
            return (int) (baseDelta * (1 + (ownerLevel / (double) 100)));
        }
