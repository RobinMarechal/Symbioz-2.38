...
namespace Symbioz.World.Providers.Fights.Spells.Sadida {
    ...
    public class Tremor : CustomSpellHandler {
        ...
        public override void Execute() {
            this.Source.OnTurnEndEvt += this.OnTurnEnded;
        }

        void OnTurnEnded(Fighter obj) {
            var maskEffect = this.GetEffect(EffectsEnum.Effect_CastSpell);

            var treeTargets = SpellEffectsManager.Instance.GetAffectedFighters(this.Source, maskEffect.GetZone(), this.CastPoint, maskEffect.TargetMask);

            foreach (var tree in treeTargets) {
                SpellEffectsManager.Instance.HandleEffects(tree, this.Level, tree.Point, this.CriticalHit);
            }
