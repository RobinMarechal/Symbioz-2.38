...
namespace Symbioz.World.Providers.Fights.Spells.Sadida {
    ...
    public class Tree : CustomSpellHandler {
        ...
        private SummonedFighter SummonedTree { get; set; }
        ...
        public override void Execute() {
            var effect = this.Level.Effects.First();
            var template = MonsterRecord.GetMonster(effect.DiceMin);
            this.SummonedTree = Summon.SummonFighter(template,
                                                     (sbyte) (template.GradeExist(this.Level.Grade) ? this.Level.Grade : template.LastGrade().Id),
                                                     this.Source,
                                                     this.CastPoint);

            SpellLevelRecord upgradeLevel = UpgradedTreeSpellRecord.GetLevel(this.Level.Grade);

            this.Source.OnTurnStartEvt += this.OnTurnStarted;
        }
