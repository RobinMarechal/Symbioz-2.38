...
namespace Symbioz.World.Providers.Fights.Results {
    ...
    public class MinationLoot : CustomLoot {
        ...
        private static ushort[] ForbiddenMonsters {
            get { return DataFile.ForbiddenMonsters.ToArray(); }
        }
        ...
        public override void Apply() {
            try {
                MonsterFighter monster = this.Result.Fighter.OposedTeam().GetFighters<MonsterFighter>(false).FindAll(x => !x.Template.IsBoss && !x.Template.IsMiniBoss && !ForbiddenMonsters.Contains(x.Template.Id)).Random();
                if (monster != null) {
                    if (new AsyncRandom().TriggerAleat(MinationDropPercentage)) {
                        this.AddMinationItem(monster);
                    }
                }

                var boss = this.Result.Fighter.OposedTeam().GetFighters<MonsterFighter>(false).FindAll(x => x.Template.IsBoss || x.Template.IsMiniBoss).Random();

                if (boss != null && new AsyncRandom().TriggerAleat(6)) {
                    this.AddMinationItem(boss);
                }
            }
            catch (Exception ex) {
        ...
        private void AddMinationItem(MonsterFighter monster) {
            CharacterItemRecord minationItem = CreateMinationItem(MinationItemTemplate, 1, this.Result.Character.Id, monster.Template, monster.GradeId);
        ...
        public class MinationDataFile {
            public List<ushort> ForbiddenMonsters { get; set; }
        }
    }
}