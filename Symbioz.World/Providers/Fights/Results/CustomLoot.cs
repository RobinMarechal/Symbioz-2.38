...
namespace Symbioz.World.Providers.Fights.Results {
    ...
    public abstract class CustomLoot {
        protected FightPlayerResult Result { get; private set; }
        ...
        protected void Add(CharacterItemRecord item) {
            this.Result.Loot.AddItem(item.GId, item.Quantity);
            this.Result.Character.Inventory.AddItem(item);
        }

        protected void Add(ushort gid, uint quantity) {
            ItemRecord template = ItemRecord.GetItem(gid);
            CharacterItemRecord item = template.GetCharacterItem(this.Result.Id, quantity, false);
            this.Add(item);
        }
