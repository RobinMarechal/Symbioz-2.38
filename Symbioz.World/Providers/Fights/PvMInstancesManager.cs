    ...
    public class PvMInstancesManager : Singleton<PvMInstancesManager> {
        ...
        public void AppendFight(Character character, int mapId, MonsterRecord[] templates) {
            if (this.Exist(mapId, character)) {
                ...
                newFight.FightStartEvt += this.newFight_FightStartEvt;
                newFight.OnFightEndedEvt += this.newFight_OnFightEndedEvt;
                newFight.StartPlacement();

                this.m_fights.Add(mapId, newFight);
            }
        }

        public void Join(Character character, int mapId) {
            character.Teleport(mapId);
            FightTeam team = this.m_fights[mapId].GetTeam(TeamTypeEnum.TEAM_TYPE_PLAYER);
        ...
        public bool Exist(int mapId, Character character) {
            return this.m_fights.ContainsKey(mapId) && this.m_fights[mapId].GetTeam(TeamTypeEnum.TEAM_TYPE_PLAYER).IsPlacementCellsFree(character.FighterCount);
        }

        void newFight_OnFightEndedEvt(Fight arg1, bool isStarted) {
            this.m_fights.Remove(arg1.Map.Id);
        }

        void newFight_FightStartEvt(Fight obj) {
            this.m_fights.Remove(obj.Map.Id);
        }
    }
}