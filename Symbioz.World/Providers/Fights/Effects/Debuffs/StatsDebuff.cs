...
namespace Symbioz.World.Providers.Fights.Effects.Debuffs {
    ...
    public class StatsDebuff : SpellEffectHandler {
        public StatsDebuff(Fighter source,
        ...
        public override bool Apply(Fighter[] targets) {
            foreach (Fighter current in targets) {
                if (this.Effect.Duration > 0) {
                    this.AddStatBuff(current, (short) -this.Effect.DiceMin, GetEffectCaracteristic(current, this.Effect.EffectEnum), FightDispellableEnum.DISPELLABLE);
                }
            }
