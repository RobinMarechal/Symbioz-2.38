...
namespace Symbioz.World.Providers.Fights.Effects {
    public abstract class SpellEffectHandler {
        protected Fighter Source { get; private set; }

        protected Fight Fight {
            get { return this.Source.Fight; }
        }

        protected SpellLevelRecord SpellLevel { get; private set; }
        protected ushort SpellId { get; private set; }
        protected EffectInstance Effect { get; private set; }
        private Fighter[] BaseTargets { get; set; }
        protected MapPoint CastPoint { get; private set; }
        protected bool Critical { get; private set; }
        ...
        public void Execute() {
            if (this.Effect.Delay > 0) {
                this.AddTriggerBuff(this.Source,
                                    FightDispellableEnum.REALLY_NOT_DISPELLABLE,
                                    TriggerType.BUFF_DELAY_ENDED,
                                    delegate(TriggerBuff buff, TriggerType trigger, object token) {
                                        this.Apply(this.BaseTargets);
        ...
        public VitalityBuff AddVitalityBuff(Fighter target, FightDispellableEnum dispelable, short num) {
            int id = target.BuffIdProvider.Pop();
            VitalityBuff buff = new VitalityBuff(id, target, this.Source, this.SpellLevel, this.Effect, this.SpellId, num, this.Critical, dispelable);
        ...
        public ShieldBuff AddShieldBuff(Fighter target, FightDispellableEnum dispelable, short num) {
            int id = target.BuffIdProvider.Pop();
            ShieldBuff buff = new ShieldBuff(id, target, this.Source, this.SpellLevel, this.Effect, this.SpellId, num, this.Critical, dispelable);
            target.AddAndApplyBuff(buff);
        ...
        public LookBuff AddLookBuff(Fighter target, FightDispellableEnum dispelable) {
            int id = target.BuffIdProvider.Pop();
            LookBuff buff = new LookBuff(id, target, this.Source, this.SpellLevel, this.Effect, this.SpellId, this.Critical, dispelable);
            target.AddAndApplyBuff(buff);
        ...
        public StateBuff AddStateBuff(Fighter target, SpellStateRecord stateRecord, FightDispellableEnum dispelable) {
            int id = target.BuffIdProvider.Pop();
            StateBuff buff = new StateBuff(id, target, this.Source, this.SpellLevel, this.Effect, this.SpellId, this.Critical, dispelable, stateRecord);
            target.AddAndApplyBuff(buff);
