...
namespace Symbioz.World.Providers.Fights.Effects.Damages {
    ...
    public class LuckyStrike : SpellEffectHandler {
        ...
        public LuckyStrike(Fighter source,
            ...
            this.Percentage = (sbyte) this.Effect.Value;
            this.HealMultiplicator = (short) this.Effect.DiceMax;
            this.DamageMultiplicator = (short) this.Effect.DiceMin;
        }

        public override bool Apply(Fighter[] targets) {
            foreach (var target in targets) {
                this.AddTriggerBuff(target, FightDispellableEnum.REALLY_NOT_DISPELLABLE, TriggerType.BEFORE_ATTACKED, this.BeforeAttacked);
            }
        ...
        private bool BeforeAttacked(TriggerBuff buff, TriggerType trigger, object token) {
            ...
            bool heal = this.Random.TriggerAleat(this.Percentage);

            if (heal) {
                buff.Target.Heal(buff.Caster, (short) (damages.Delta * this.HealMultiplicator));

                return true;
            }
            else {
                damages.Delta *= this.DamageMultiplicator;
