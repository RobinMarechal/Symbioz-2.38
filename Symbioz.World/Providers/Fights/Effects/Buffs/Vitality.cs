...
namespace Symbioz.World.Providers.Fights.Effects.Buffs {
    ...
    public class VitalityPercent : SpellEffectHandler {
        public VitalityPercent(Fighter source,
        ...
        public override bool Apply(Fighter[] targets) {
            foreach (Fighter current in targets) {
                double num = current.Stats.MaxLifePoints * (this.Effect.DiceMin / 100.0);
    ...
    public class Vitality : SpellEffectHandler {
        public Vitality(Fighter source,
                        SpellLevelRecord spellLevel,
                        EffectInstance effect,
                        Fighter[] targets,
                        MapPoint castPoint,
        ...
        public override bool Apply(Fighter[] targets) {
            foreach (Fighter current in targets) {
                this.AddVitalityBuff(current, FightDispellableEnum.DISPELLABLE, (short) this.Effect.DiceMin);
            }
