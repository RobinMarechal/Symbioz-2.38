...
namespace Symbioz.World.Providers.Fights.Effects.Buffs {
    ...
    public class StatsBuff : SpellEffectHandler {
        public StatsBuff(Fighter source,
        ...
        public override bool Apply(Fighter[] targets) {
            foreach (Fighter current in targets) {
                if (this.Effect.Duration > 0) {
                    this.AddStatBuff(current, (short) this.Effect.RandomizeMinMax(), GetEffectCaracteristic(current, this.Effect.EffectEnum), FightDispellableEnum.DISPELLABLE);
                }
            }
