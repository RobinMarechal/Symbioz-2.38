        ...
        public override bool Apply(Fighter[] targets) {
            foreach (Fighter current in targets) {
                if (this.Effect.Duration > 0) {
                    this.AddStatBuff(current, (short) this.Effect.DiceMin, current.Stats.MovementPoints, FightDispellableEnum.DISPELLABLE);
                }
                else {
                    current.RegainMp(this.Source.Id, (short) this.Effect.DiceMin);
                }
            }
