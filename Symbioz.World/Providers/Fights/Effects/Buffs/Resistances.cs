...
namespace Symbioz.World.Providers.Fights.Effects.Buffs {
    ...
    public class Resistances : SpellEffectHandler {
        public Resistances(Fighter source,
        ...
        public override bool Apply(Fighter[] targets) {
            foreach (Fighter target in targets) {
                if (this.Effect.Duration > 0) {
                    ResistancesBuff buff = new ResistancesBuff(target.BuffIdProvider.Pop(), target, this.Source, this.SpellLevel, this.Effect, this.SpellId, this.Critical, FightDispellableEnum.DISPELLABLE, (short) this.Effect.DiceMin);
