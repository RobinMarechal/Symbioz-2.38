...
namespace Symbioz.World.Providers.Fights.Effects.Movements {
    ...
    public class Friction : SpellEffectHandler {
        private short CellsStep { get; set; }

        public Friction(Fighter source,
            ...
            this.CellsStep = (short) this.Effect.DiceMax;
        }

        public override bool Apply(Fighter[] targets) {
            foreach (var target in targets) {
                this.AddTriggerBuff(target, 0, TriggerType.AFTER_ATTACKED, this.AfterAttacked);
            }
        ...
        private bool AfterAttacked(TriggerBuff buff, TriggerType trigger, object token) {
            ...
            if (buff.Target.Point.IsInLine(damages.Source.Point)) {
                buff.Target.Abilities.PullForward(damages.Source, this.CellsStep, buff.Target.Point);
            }
