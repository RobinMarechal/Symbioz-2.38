...
namespace Symbioz.World.Providers.Fights.Effects.Others {
    ...
    public class AddState : SpellEffectHandler {
        public AddState(Fighter source,
        ...
        public override bool Apply(Fighter[] targets) {
            SpellStateRecord stateRecord = SpellStateRecord.GetState(this.Effect.Value);

            if (stateRecord != null) {
                foreach (var target in targets) {
                    this.AddStateBuff(target, stateRecord, Protocol.Enums.FightDispellableEnum.DISPELLABLE_BY_STRONG_DISPEL);
                }
