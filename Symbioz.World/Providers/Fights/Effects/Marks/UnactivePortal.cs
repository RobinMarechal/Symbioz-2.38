...
namespace Symbioz.World.Providers.Fights.Effects.Marks {
    ...
    public class UnactivePortal : SpellEffectHandler {
        ...
        public override bool Apply(Fighter[] targets) {
            var portal = this.Fight.GetMarks<Portal>(x => x.CenterPoint.CellId == this.CastPoint.CellId).FirstOrDefault();

            if (portal != null && portal.Source == this.Source) {
                portal.Unactive(this.Source);
            }
