...
namespace Symbioz.World.Providers.Fights.Effects.Summons {
    ...
    public class Summon : SpellEffectHandler {
        ...
        public override bool Apply(Fighter[] targets) {
            var template = MonsterRecord.GetMonster(this.Effect.DiceMin);
            SummonFighter(template,
                          (sbyte) (template.GradeExist(this.SpellLevel.Grade) ? this.SpellLevel.Grade : template.LastGrade().Id),
                          this.Source,
                          this.CastPoint);
