...
namespace Symbioz.World.Providers.Fights.Effects.Summons {
    ...
    public class Revive : SpellEffectHandler {
        private Fighter RevivedFighter { get; set; }

        public Revive(Fighter source,
        ...
        public override bool Apply(Fighter[] targets) {
            Fighter target = this.Source.Team.LastDead();

            if (target != null) {
                this.RevivedFighter = target;
                this.Fight.ReviveFighter(this.Source, target, this.CastPoint.CellId, (short) this.Effect.DiceMin);
                this.Source.BeforeDeadEvt += this.Source_OnDeadEvt;
            }
        ...
        void Source_OnDeadEvt(Fighter obj) {
            this.RevivedFighter.Die(this.Source);
        }
    }
}