...
namespace Symbioz.World.Providers.Fights.Effects.Movements {
    ...
    public class RepelsTo : SpellEffectHandler {
        public RepelsTo(Fighter source,
        ...
        public override bool Apply(Fighter[] targets) {
            DirectionsEnum orientation = this.Source.Point.OrientationTo(this.CastPoint, true); //Source.Point=> this.TargetedPoint
            short adjCellId = this.Source.Point.GetCellInDirection(orientation, 1).CellId;
            ...
            if (target == null) {
                ...
                MapPoint[] cellsOnLineBetween = startPoint.GetCellsOnLineBetween(this.CastPoint);
                for (int index = 0; index < cellsOnLineBetween.Length; ++index) {
                    ...
                    if (!this.Fight.IsCellFree(mapPoint.CellId))
                        point = index <= 0 ? startPoint : new MapPoint(cellsOnLineBetween[index - 1].CellId);
                ...
                target.Slide(this.Source, point.CellId);
