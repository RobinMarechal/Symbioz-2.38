...
namespace Symbioz.World.Providers.Fights.Effects {
    public class PortalProvider : Singleton<PortalProvider> {
        ...
        public Tuple<Portal, Portal> GetPortalsTuple(Fight fight, short cellId) {
            ...
            return new Tuple<Portal, Portal>(this.GetPortal(enter.Source, (short) res.First()), this.GetPortal(enter.Source, (short) res.Last()));
        }
        ...
        private MapPoint getBestNextPortal(MapPoint refCell, List<MapPoint> closests) {
            ...
            closests.Sort(delegate(MapPoint o1, MapPoint o2) {
                double tap = this.GetOrientedAngle(refCoord, nudge, new Point(o1.X, o1.Y)) - this.GetOrientedAngle(refCoord, nudge, new Point(o2.X, o2.Y));
        ...
        private double GetAngle(Point coordRef, Point coordA, Point coordB) {
            var a = this.GetDistance(coordA, coordB);
            var b = this.GetDistance(coordRef, coordA);
            var c = this.GetDistance(coordRef, coordB);
