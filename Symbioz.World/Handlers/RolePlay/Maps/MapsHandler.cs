...
namespace Symbioz.World.Handlers.RolePlay {
    class MapsHandler {
        ...
        public static void HandleMapMovementConfirm(GameMapMovementConfirmMessage message, WorldClient client) {
            ...
            var newCellId = client.Character.CellId;
            var mapId = client.Character.Map.Id;

            // Handle exit cells
            var exitCell = InteractiveElementRecord.InteractiveElements.Find(el => el.MapId == mapId && el.CellId == newCellId && el.ElementType == MapRecord.MAP_EXIT_INTERACTIVE);

            if (exitCell == null) return;
            {
                var exitCellSkill = InteractiveSkillRecord.InteractiveSkills.Find(el => el.ElementId == exitCell.ElementId && el.ActionType.Equals("Teleport"));
                client.Character.Teleport(int.Parse(exitCellSkill.Value1), ushort.Parse(exitCellSkill.Value2));
            }
        }
