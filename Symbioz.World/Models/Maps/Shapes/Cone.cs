...
namespace Symbioz.World.Models.Maps.Shapes {
    public class Cone : IShape {
        public uint Surface {
            get { return (uint) ((this.Radius + 1) * (this.Radius + 1)); }
        }
                    ...
                    case DirectionsEnum.DIRECTION_SOUTH_EAST:
                        for (int i = mapPoint.X; i <= mapPoint.X + (int) this.Radius; i++) {
                            for (int j = -num; j <= num; j++) {
                                if (this.MinRadius == 0 || Math.Abs(mapPoint.X - i) + Math.Abs(j) >= this.MinRadius) {
                    ...
                    case DirectionsEnum.DIRECTION_SOUTH_WEST:
                        for (int j = mapPoint.Y; j >= mapPoint.Y - (int) this.Radius; j--) {
                            for (int i = -num; i <= num; i++) {
                                if (this.MinRadius == 0 || Math.Abs(i) + Math.Abs(mapPoint.Y - j) >= this.MinRadius) {
