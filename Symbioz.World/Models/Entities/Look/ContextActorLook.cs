...
using YAXLib;
using Extensions = Symbioz.Core.Extensions;
...
namespace Symbioz.World.Models.Entities.Look {
    ...
    public class ContextActorLook {
        ...
        public List<ushort> Skins { get; private set; }
        ...
        public short Scale {
            get { return (short) (this.Scales.Count == 0 ? 100 : this.Scales[0]); }
        }

        public List<ContextSubEntity> SubEntities { get; private set; }

        public bool IsRiding {
            get { return this.SubEntities.Find(x => x.Category == SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER) != null; }
        }
        ...
        public ContextActorLook(ushort bonesId,
        ...
        public void SetScale(short value) {
            ...
            if (look.Scales.Count == 0) {
                ...
                this.ActiveLook().Scales[0] = value;
            }
        }

        public void RemoveSkin(ushort value) {
            this.ActiveLook().Skins.RemoveAll(x => x == value);
        }

        public void SetBones(ushort value) {
            this.ActiveLook().BonesId = value;
        }

        public void SetColors(IEnumerable<int> colors) {
            this.ActiveLook().Colors = colors.ToList();
        }
        ...
        public ContextSubEntity GetSubEntity(SubEntityBindingPointCategoryEnum category) {
            return this.SubEntities.Find(x => x.Category == category);
        }

        public void AddPetSkin(ushort skinId, short scale) {
            this.ActiveLook().SubEntities.Add(new ContextSubEntity(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_PET, 0, BonesLook(skinId, scale)));
        }

        public void RemovePetSkin() {
            this.ActiveLook().SubEntities.RemoveAll(x => x.Category == SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_PET);
        }
        ...
        public ContextActorLook GetMountLook(ContextActorLook mountLook) {
            ...
            ContextSubEntity actorSub = new ContextSubEntity(SubEntityBindingPointCategoryEnum.HOOK_POINT_CATEGORY_MOUNT_DRIVER,
                                                             0,
        ...
        public override bool Equals(object obj) {
            return obj != null
                   && obj is ContextActorLook look
                   && look.BonesId == this.BonesId
                   && look.Colors.SequenceEqual(look.Colors)
                   && look.Scales.SequenceEqual(this.Scales)
                   && look.Skins.SequenceEqual(this.Skins)
        ...
        public ContextActorLook ActiveLook() {
            if (!this.IsRiding)
                ...
                return this.GetMountDriverLook();
        }

        public EntityLook ToEntityLook() {
            return new EntityLook(this.BonesId, this.Skins.ToArray(), this.Colors.ToArray(), this.Scales.ToArray(), this.SubEntities.ConvertAll(x => x.ToSubEntity()).ToArray());
        }
        ...
        public static ContextActorLook Parse(string str) {
            ...
            int num = str.IndexOf('|');
            if (num == -1) {
                num = str.IndexOf("}", StringComparison.Ordinal);
                if (num == -1) {
                    throw new Exception("Incorrect EntityLook format : " + str);
                }
            }
            ...
            if ((num = str.IndexOf('|', i)) != -1 || (num = str.IndexOf('}', i)) != -1) {
                skins = Extensions.ParseCollection(str.Substring(i, num - i), short.Parse);
                i = num + 1;
            }

            Tuple<int, int>[] source = new Tuple<int, int>[0];
            if ((num = str.IndexOf('|', i)) != -1 || (num = str.IndexOf('}', i)) != -1) {
                source = Extensions.ParseCollection(str.Substring(i, num - i), ParseIndexedColor);
                i = num + 1;
            }

            short[] scales = new short[0];
            if ((num = str.IndexOf('|', i)) != -1 || (num = str.IndexOf('}', i)) != -1) {
                scales = Extensions.ParseCollection(str.Substring(i, num - i), short.Parse);
                i = num + 1;
            }

            List<ContextSubEntity> list = new List<ContextSubEntity>();
        ...
        public static string ConvertToString(ContextActorLook look) {
            ...
            if (look.Scales == null) {
                ...
                stringBuilder.Append("|".ConcatCopy(num + 1));
                num = 0;
                stringBuilder.Append(string.Join(",", look.Scales));
            }

            if (!look.SubEntities.Any()) {
                num++;
            }
                ...
                foreach (var sub in look.SubEntities) {
                    StringBuilder subBuilter = new StringBuilder();
        ...
        public static int[] GetConvertedColors(IEnumerable<int> colors) {
            var colorsArray = colors as int[] ?? colors.ToArray();
            int[] col = new int[colorsArray.Count()];
            for (int i = 0; i < colorsArray.Count(); i++) {
                var color = Color.FromArgb(colorsArray[i]);
        ...
        public ContextActorLook Clone() {
            ...
            return new ContextActorLook(this.BonesId,
                                        new List<ushort>(this.Skins),
                                        new List<int>(this.Colors),
                                        new List<short>(this.Scales),
                                        new List<ContextSubEntity>(subentities));
        }
    }
