...
namespace Symbioz.World.Models.Fights.FightModels {
    public class Synchronizer {
        ...
        public event Action<Synchronizer> Success;
        public event Action<Synchronizer, PlayableFighter[]> Timeout;

        private void NotifySuccess() {
            Action<Synchronizer> success = this.Success;
            if (success != null) {
                success(this);
            }
        }
            ...
            if (!this.m_started) {
                ...
                if (this.m_fighters.Count <= 0) {
                    ...
                    foreach (CharacterFighter current in this.m_fighters) {
                        current.Character.Client.Send(new GameFightTurnReadyRequestMessage(this.m_fight.FighterPlaying.Id));
                    }

                    this.m_timer = new ActionTimer(CheckTimeout, this.TimedOut, false);
                    this.m_timer.Start();
                }
            }
        }
        ...
        public static Synchronizer RequestCheck(Fight fight, Action success, Action<PlayableFighter[]> fail) {
        ...
        public static Synchronizer RequestCheck(Fight fight, Action success, List<PlayableFighter> fighters, Action<PlayableFighter[]> fail) {
            Synchronizer readyChecker = new Synchronizer(fight, fighters);
            readyChecker.Success += delegate(Synchronizer chk) { success(); };
            readyChecker.Timeout += delegate(Synchronizer chk, PlayableFighter[] laggers) { fail(laggers); };
            readyChecker.Start();
