...
using Portal = Symbioz.World.Models.Fights.Marks.Portal;
...
namespace Symbioz.World.Models.Fights.Fighters {
    public abstract class Fighter {
        ...
        public ContextActorLook RealLook { get; private set; }
        ...
        public short TurnStartCell { get; set; }
        public short FightStartCell { get; protected set; }

        public DateTime? TurnStartTime { get; set; }
        public DateTime? DeathTime { get; set; }

        public MapPoint Point {
            get { return new MapPoint((short) CellId); }
        }

        public ushort MapCellId { get; set; }

        public DirectionsEnum Direction { get; set; }

        public bool IsSummon {
            get { return Stats.Summoned; }
        }
        ...
        public abstract bool Sex { get; }
        public Abilities Abilities { get; private set; }

        protected List<SpellStateRecord> States { get; set; }
        public Fighter Carried { get; set; }
        public Fighter Carrying { get; set; }

        public bool CarryFighter {
            get { return Carried != null; }
        }

        public bool IsCarried {
            get { return Carrying != null; }
        }

        public bool CantSwitchPosition {
            get { return this.HasState(x => x.CantSwitchPosition); }
        }

        public bool IsInvulnerable {
            get { return HasState(x => x.Invulnerable); }
        }

        public bool InvulnerableMelee {
            get { return HasState(x => x.InvulnerableMelee); }
        }

        public bool InvunlerableRange {
            get { return HasState(x => x.InvulnerableRange); }
        }

        public bool Incurable {
            get { return HasState(x => x.Incurable); }
        }
        ...
        public bool IsTeamLeader {
            get { return Team.Leader == this; }
        }

        public Loot Loot { get; set; }

        public bool IsFighterTurn {
            get { return this.Fight.TimeLine.Current == this; }
        }
        ...
        public UniqueIdProvider BuffIdProvider { get; private set; }
        ...
        public int SummonCount {
            get { return Team.GetFighters<IOwnable>(true).FindAll(x => x.IsOwner(this)).Count; }
        }

        public bool CanSummon {
            get { return Stats.SummonableCreaturesBoost.TotalInContext() > SummonCount; }
        }

        private Dictionary<ushort, short> BuffedSpells { get; set; }

        public bool BlockSight {
            get { return Alive && Stats.InvisibilityState != GameActionFightInvisibilityStateEnum.INVISIBLE; }
        }
        ...
        public bool BuffMaxStackReached(Buff buff) {
            return buff.Level.MaxStacks > 0
                   && buff.Level.MaxStacks
                   <= this.Buffs.Count((Buff entry) => entry.SpellId == buff.SpellId
                                                       && entry.Effect.EffectId == buff.Effect.EffectId
                                                       && entry.Effect.Delay == buff.Effect.Delay
        ...
        public SpellCastResultEnum CanCastSpell(SpellLevelRecord level, short sourceCellId, short cellId) {
            ...
            if (!castZone.Contains(cellId)) {
                if (this is PlayableFighter) {
                    PlayableFighter characterFighter = this as PlayableFighter;
                    characterFighter?.GetCharacterPlaying().ReplyError("This cell is too far.");
                }
        ...
        public void ForceSpellCast(SpellLevelRecord level, short cellId) {
            Fight.Send(new GameActionFightSpellCastMessage(0,
                                                           Id,
                                                           false,
                                                           false,
                                                           0,
        ...
        public virtual bool CastSpell(SpellRecord spell, sbyte grade, short cellId, int targetId = 0, bool verif = true) {
            ...
            if (level != null) {
                    ...
                    if (PortalProvider.Instance.HasPortail(Fight, cellId)) {
                        if (Fight.IsCellFree(cellId)) {
                            ...
                            if (two != null && two.Item2 != null) {
                                ...
                                this.Stats.FinalDamageCoefficient =
                                    (start != null ? (1d + ((double) start.SpellLevel.Effects.First().Value + 2d * (double) PortalProvider.Instance.GetCasesCount(Fight, (short) start.CenterPoint.CellId)) / 100d) : 0);
                            }
                        }
                    }
                }
                ...
                Fight.Send(new GameActionFightSpellCastMessage((ushort) ActionsEnum.ACTION_FIGHT_CAST_SPELL,
                                                               Id,
                                                               false,
                                                               true,
                                                               targetId,
        ...
        public void Carry(Fighter source, SpellEffectHandler handler) {
            ...
            Fight.Send(new GameActionFightCarryCharacterMessage((ushort) ActionsEnum.ACTION_CARRY_CHARACTER,
                                                                source.Id,
                                                                this.Id,
                                                                this.CellId));
            this.CellId = source.CellId;
        ...
        public virtual IEnumerable<DroppedItem> RollLoot(int teamPp, int dropBonusPercent) {
        ...
        public abstract FightTeamMemberInformations GetFightTeamMemberInformation();
