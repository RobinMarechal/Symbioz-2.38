...
namespace Symbioz.World.Providers.Maps.PlacementPattern {
    ...
    public class PlacementComplexityCalculator {
        ...
        public int Compute() {
            PointsGroup[] groups = this.GetPointsGroups();
            int result;
            if (groups.Length == 0) {
                result = 0;
            }
                ...
                List<PointsGroup> exclusions = new List<PointsGroup>();
                PointsGroup[] array = groups;
                PointsGroup group;
                for (int i = 0; i < array.Length; i++) {
                    group = array[i];
                    distanceSum += (from entry in groups
                                    where !exclusions.Contains(entry)
                                    select entry).Sum((PointsGroup entry) => this.DistanceTo(entry.Center, group.Center));
                    exclusions.Add(group);
                }

                double distanceAverage = distanceSum / groups.Length;
                int counts = this.m_points.Length;
                result = (int) (counts * distanceAverage + groups.Length * groups.Average((PointsGroup entry) => entry.Points.Length));
            }
            ...
            for (int i = 0; i < points.Length; i++) {
                ...
                if (!exclusions.Contains(point)) {
                    ...
                    if (group.Length > 0) {
                        ...
                        result.Add(new PointsGroup(group, this.GetCenter(group)));
                    }
                }
            }
